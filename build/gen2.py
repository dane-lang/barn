    ...
    return [
        ('POSTS & FOUNDATION',
         [('8x8 treated post', f"{ft(S['POST_LEN'])} · ground-contact rated (UC4B)", n_post, 'ea', '10 field + 4 roll-up jamb'),
          ...
          ('Treated 2x10 splash plank', '16\'', 8, 'ea', f"perimeter {2*(side_len+end_len):.0f} LF")]),
        ('WALL FRAMING',
         [('2x6 girts', '16\' · #2 SPF', int(girt_lf / 16) + 2, 'ea', f"{girt_rows} runs × perimeter = {girt_lf:.0f} LF"),
        ...
        ('ROOF',
         [('Kilby T02 fink truss', "6:12 · 20' span · 2' tails", 3, 'ea', 'T2 T3 T4'),
          ...
          ('2x6 purlins', '16\' · flat @ 24" o.c.', int(purlin_lf / 16) + 2, 'ea', f"~{purlin_lf:.0f} LF"),
